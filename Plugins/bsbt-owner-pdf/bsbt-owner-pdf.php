...
final class BSBT_Owner_PDF {
    ...
    private static function collect_data(int $booking_id): array {
        ...
        if (function_exists('get_field')) {
            ...
            if (!empty($acf_owner)) $owner_name = trim($acf_owner);
        }
        if ($owner_name === '—') {
            $meta_owner = trim((string)get_post_meta($rt, 'owner_name', true));
            if ($meta_owner !== '') $owner_name = $meta_owner;
        }
        ...
        $pricing = null;
        $final_payout_raw = 0.0;

        if ($model_key === 'model_b' && function_exists('bsbt_get_model_b_pricing_view')) {
            $pricing = bsbt_get_model_b_pricing_view($booking_id);
            if ($pricing) {
                $final_payout_raw = (float)$pricing['owner_net_total'];
            }
        }

        // Fallback for Model A or if pricing failed
        if ($final_payout_raw <= 0) {
            $ppn = (float)get_post_meta($rt, 'owner_price_per_night', true);
            $final_payout_raw = $ppn * $n;
        }
        ...
        return [
            ...
            'data'=>[
                ...
                'payout'=>number_format($final_payout_raw, 2, ',', '.'),
                'pricing'=>$pricing,
            ]
        ];
    }
